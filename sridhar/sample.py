...
def search():
    ...
    df['combined_data'] = df[df.columns[4:]].apply(lambda x: ' '.join(x.dropna().astype(str)),axis=1)
    ...
    def recommender(keyword):
    #earch_keyword = request.form['keyword']
    ...
    #create enumerations for similarity scores and sort it 
        ...
        for item in sorted_score:
            ...
            results.append(product_namef+'  '+ str(sorted_score[i][1]))
            i = i+1
            if i == 5:
                break
            return results
